...
class ToxicDataset(Dataset):
    ...
    def __init__(self, tokenizer: DistilBertTokenizer, dataframe: pd.DataFrame, lazy: bool = False):
    ...
    @staticmethod
    def row_to_tensor(tokenizer: DistilBertTokenizer, row: pd.Series) -> Tuple[torch.LongTensor, torch.LongTensor]:
...
class DistilBertClassifier(nn.Module):
    ...
    def __init__(self, bert: DistilBertModel, num_classes: int):
        super().__init__()
        self.bert = bert
        self.classifier = nn.Linear(bert.config.dim, num_classes)
    ...
    def forward(self, input_ids, attention_mask=None, position_ids=None, head_mask=None,
        ...
        cls_output = last_hidden_state[:, 0, :] # batch, hidden
...
def inference_db(model, submission, test_df, bert_model_name = 'distilbert-base-cased', BATCH_SIZE = 32, path = False, data_path = None, csv = False):
...
if __name__ == "__main__":
    ...
    submission.to_csv("submission_dbert.csv", index=False)
